...
const { zipSync, zip } = require("cross-zip")
...
class MakerInnoSetup extends MakerBase {
  ...
  async make({ dir, makeDir, targetArch, packageJSON, appName, forgeConfig }) {
    //console.info(dir, makeDir, targetArch, appName, forgeConfig)

    const outPath = path.resolve(makeDir, `innosetup/${targetArch}/${packageJSON.version}`)
    await this.ensureDirectory(outPath);

    const innoSetupScript = targetArch === "ia32" ? "innosetup32.iss" : "innosetup64.iss"
...
async function createWindowsInstaller(config) {
  let cmd = 'C:\\Program Files (x86)\\Inno Setup 6\\ISCC.exe'
